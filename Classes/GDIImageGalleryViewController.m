...
@interface GDIImageGalleryViewController () {
    BOOL _isActive;
	BOOL _isFullscreen;
	BOOL _isScrolling;
...
- (void)loadView
{
	...
	UIImage *leftIcon = [UIImage imageNamed:@"GDIImageGallery.bundle/photo-gallery-left.png"];
	UIImage *rightIcon = [UIImage imageNamed:@"GDIImageGallery.bundle/photo-gallery-right.png"];
...
- (void)destroyViews {
    ...
    for (int i=0; i<[photoKeys count]; i++) {
        GDIImageGalleryPhoto *photoLoader = [_photoLoaders objectForKey:[photoKeys objectAtIndex:i]];
...
- (void)buildViews
{
	...
	for (i = 0; i < count; i++) {
		GDIImageGalleryPhotoView *photoView = [[GDIImageGalleryPhotoView alloc] initWithFrame:CGRectZero];
...
- (void)buildThumbsViewPhotos
{
	...
	for (i = 0; i < count; i++) {
		...
		GDIImageGalleryPhotoView *thumbView = [[GDIImageGalleryPhotoView alloc] initWithFrame:CGRectZero target:self action:@selector(handleThumbClick:)];
...
- (void)arrangeThumbs
{
	...
	for (i = 0; i < count; i++) {
		GDIImageGalleryPhotoView *thumbView = [_photoThumbnailViews objectAtIndex:i];
...
- (void)didReceiveMemoryWarning {
	...
	NSLog(@"[GDIImageGalleryViewController] didReceiveMemoryWarning! clearing out cached images...");
...
{
	if([self.visibleViewController isKindOfClass:[GDIImageGalleryViewController class]]) 
	{
        return YES;
	}
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
	...
	if([self.visibleViewController isKindOfClass:[GDIImageGalleryViewController class]])
	{
		GDIImageGalleryViewController *galleryController = (GDIImageGalleryViewController*)self.visibleViewController;
...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    if( [self.selectedViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        // see if the current controller in the stack is a gallery
        if([navController.visibleViewController isKindOfClass:[GDIImageGalleryViewController class]])
        {
            return YES;
        }
    }
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
	if([self.selectedViewController isKindOfClass:[UINavigationController class]])
	{
		...
		if([navController.visibleViewController isKindOfClass:[GDIImageGalleryViewController class]])
		{
			GDIImageGalleryViewController *galleryController = (GDIImageGalleryViewController*)navController.visibleViewController;
			[galleryController resetImageViewZoomLevels];
		}
	}
}
